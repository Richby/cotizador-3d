...
const SceneContent = ({ geometry, setDimensions, scale }: ModelViewerProps) => {
  const { camera } = useThree(); // ✅ useThree() DENTRO de SceneContent
  ...
  useEffect(() => {
    if (geometry) {
      ...
      if (box) {
        ...
        if (meshRef.current) {
          meshRef.current.position.y = height / 2; // Mover el mesh hacia arriba
          meshRef.current.scale.set(scale, scale, scale); // Aplicar la escala al mesh
        }

        // 4. Ajuste de la Cámara (Ortográfica)
        const size = box.getSize(new THREE.Vector3());
        const maxSize = Math.max(size.x, size.z); // Usamos X y Z para la vista superior
        const aspect = size.x / size.z; // El aspect ratio del MODELO
        ...
        if (camera instanceof THREE.OrthographicCamera) {
            ...
            camera.position.set(0, maxSize * 2, 0); // Posición encima, distancia proporcional
            camera.lookAt(0, height / 2, 0); // Mirar al centro (altura/2)
  ...
  }, [geometry, setDimensions, camera, scale]); // Dependencias: geometry, setDimensions, camera, scale
...
const ModelViewer: React.FC<ModelViewerProps> = ({ geometry, setDimensions, scale }) => { // Recibe scale
  ...
  // const [height, setHeight] = useState<string | number>("auto"); // Ya no es necesario
  ...
  return (
    <div ref={containerRef} className="mt-6 bg-gray-200 rounded-md overflow-hidden aspect-square">
      ...
      <Canvas shadows orthographic camera={{ position: [0, 10, 0], zoom: 50 }}>
        {/* Pasar la escala a SceneContent */}
